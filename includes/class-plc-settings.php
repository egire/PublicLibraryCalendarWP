...
class PLC_Settings {
	...
	public static function sanitize( $input ) {
		...
		$out['show_qr']      = empty( $input['show_qr'] ) ? 0 : 1;
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table" role="presentation">
					...
					<tr>
						<th scope="row"><?php esc_html_e( 'Event QR code', 'plc' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="<?php echo esc_attr( self::name( 'show_qr' ) ); ?>" value="1" <?php checked( $s['show_qr'], 1 ); ?>>
								<?php esc_html_e( 'Show a scannable QR code on each event page', 'plc' ); ?>
							</label>
							<p class="description"><?php esc_html_e( 'Patrons can scan it to open the event on their phone. The image is generated by a free QR service and loads lazily; developers can swap it via the plc_qr_image_url filter.', 'plc' ); ?></p>
						</td>
					</tr>
				</table>
